...
// Add HttpClient for the aggregation endpoint
builder.Services.AddHttpClient("orderservice", c =>
    c.BaseAddress = new Uri("http://orderservice:8080/"));
builder.Services.AddHttpClient("customerservice", c =>
    c.BaseAddress = new Uri("http://customerservice:8080/"));
...
// Aggregation endpoint: GET /api/aggregate/orders/{id}
// Returns order + customer + product details in a single response
app.MapGet("/api/aggregate/orders/{id:int}", async (int id, IHttpClientFactory factory) =>
{
    ...
    var order = await orderResp.Content.ReadFromJsonAsync<dynamic>();

    // We need the raw JSON to extract fields
    var orderJson = System.Text.Json.JsonDocument.Parse(
        await orderClient.GetStringAsync($"api/orders/{id}"));
